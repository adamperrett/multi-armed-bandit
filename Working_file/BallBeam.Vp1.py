...
input_labels = list()
output_labels = list()
big_input_labels = list()
big_output_labels = list()
no_input_pop = 2
no_output_pop = 2
...
max_angle = np.pi/4
min_angle = -max_angle
current_agent = [0 for i in range(parallel_agents)]
current_angle = [0 for i in range(parallel_agents)]
current_position = [0 for i in range(parallel_agents)]
current_beam_vlct= [0 for i in range(parallel_agents)]
current_ball_vlct = [0 for i in range(parallel_agents)]
current_beam_acc = [0 for i in range(parallel_agents)]
current_ball_acc = [0 for i in range(parallel_agents)]
...
motor_spikes = [[0 for i in range(no_output_pop)] for j in range(parallel_agents)]
def receive_spikes(label, time, neuron_ids):
    for neuron_id in neuron_ids:
        for i in range(parallel_agents):
            for j in range(no_output_pop):
                if label == big_output_labels[i][j]:
                    motor_spikes[i][j] += 1
                    #print "\n\nmotor {} - time = {}\n\n".format(i, time)
...
def poisson_setting(label, connection):
    ...
    global motor_spikes
    for i in range(parallel_agents):
        if connection._send_labels[0] == "{}_control".format(big_input_labels[i][0]):
            agent = i
            break
    #convert to poisson rate
    min_poisson_dist = map_pop[current_agent[agent]][input_poisson_low]
    max_poisson_dist = map_pop[current_agent[agent]][input_poisson_high]
    min_poisson_angle = map_pop[current_agent[agent]][input_poisson_low+no_input_pop]
    max_poisson_angle = map_pop[current_agent[agent]][input_poisson_high+no_input_pop]
    ...
    for i in range(0, duration, time_segments):
        ...
        for j in range(no_output_pop):
            ...
            if j < no_output_pop / 2:
                clockwise += motor_spikes[agent][j]
                motor_spikes[agent][j] = 0
            #anticlockwise
            else:
                anticlock += motor_spikes[agent][j]
                motor_spikes[agent][j] = 0
        total_clock = clockwise - anticlock
        torque = float(total_clock) * spike_to_torque
        current_ball_acc[agent] = (current_ball_vlct[agent]*np.power(current_beam_vlct[agent],2)) - (g*np.sin(current_angle[agent]))
        current_ball_acc[agent] /= (1 + (moi_ball/(mass*np.power(radius,2))))
        # current_ball_acc[agent] = (-5.0/7.0) * ((g * np.sin(current_angle[agent])) - (current_position[agent] * np.power(current_beam_vlct[agent], 2)))
        current_beam_acc[agent] = (torque - (mass*g*current_position[agent]*np.cos(current_angle[agent])) -
                            (2*mass*current_position[agent]*current_ball_vlct[agent]*current_beam_vlct[agent]))
        #current_beam_acc[agent] = torque
        current_beam_acc[agent] /= (mass*np.power(current_position[agent],2)) + moi_ball + moi_beam
        seconds_window = float(time_segments / 1000.0)
        current_ball_vlct[agent] += float(current_ball_acc[agent] * seconds_window)
        current_position[agent] += float(current_ball_vlct[agent] *seconds_window)
        current_beam_vlct[agent] += float(current_beam_acc[agent] * seconds_window)
        #check that the beam is not at the maximum angle
        previous_angle = current_angle[agent]
        current_angle[agent] += float(current_beam_vlct[agent] * seconds_window)
        current_angle[agent] = max(min(current_angle[agent], max_angle), min_angle)
        if current_angle[agent] == previous_angle:
            current_beam_acc[agent] = 0
            current_beam_vlct[agent] = 0
        print "clock = {}\tanti = {}\ttorque = {}\tpos = {}\tangle = {}".format(clockwise,anticlock,torque,current_position[agent],current_angle[agent])
        #set poisson rate
        current_pos_ratio = max(min((current_position[agent] + beam_length) / (beam_length * 2), 1), 0)
        poisson_position = min_poisson_dist + ((max_poisson_dist - min_poisson_dist) * current_pos_ratio)
        current_ang_ratio = (current_angle[agent] + max_angle) / (max_angle * 2)
        ...
        connection.set_rates(input_labels[agent][0], [(i, int(round(poisson_position))) for i in range(n_number)])
        n_number = map_pop[agent][map_neuron_count+map_neuron_params]
        connection.set_rates(input_labels[agent][1], [(i, int(round(poisson_angle))) for i in range(n_number)])
        big_experimental_record[agent].append([current_position[agent], current_ball_vlct[agent],current_ball_acc[agent],
                                    current_angle[agent], current_beam_vlct[agent], current_beam_acc[agent], time.clock()])
        if abs(current_position[agent]) > beam_length or time.clock() - start > float(duration) / 1000.0:
            #p.end()
            break
        finish = time.clock()
...
#creating multiple parallel agents
def create_spinn_nets(agent):
    global input_labels
    global output_labels
    global big_input_labels
    global big_output_labels
    ...
    if port_offset !=0:
        for i in range(parallel_agents):
            del big_input_labels[0]
            del big_output_labels[0]
    for i in range(parallel_agents):
        create_spinn_net(agent+i)
        big_input_labels.append(input_labels)
        big_output_labels.append(output_labels)
    ...
    for i in range(max_neuron_types):
        ...
        if i < no_input_pop:
            if held_input_pop_size != 0:
                ...
                n_number = map_pop[agent][map_neuron_count + n_selected]
            n_pop_labels.append("Input_pop{}/{}-index{}".format(i, agent, n_index))
            input_labels.append("Input_pop{}/{}-index{}".format(i, agent, n_index))
            n_pop_list.append(p.Population(n_number, p.SpikeSourcePoisson(rate=input_poisson_min), label=n_pop_labels[i]))
            p.external_devices.add_poisson_live_rate_control(n_pop_list[i],database_notify_port_num=16000+agent+port_offset)
        #set up output pop
        elif i < no_input_pop + no_output_pop:
            n_pop_labels.append("Output_pop{}/{}-index{}".format(i-no_input_pop, agent, n_index))
            output_labels.append("Output_pop{}/{}-index{}".format(i-no_input_pop, agent, n_index))
            n_pop_list.append(p.Population(n_number, p.IF_cond_exp(), label=n_pop_labels[i]))
            p.external_devices.activate_live_output_for(
                n_pop_list[i], database_notify_port_num=18000+agent+port_offset, port=14000+agent+port_offset)
        #set up all other populations
        else:
            n_pop_labels.append("neuron{}/{}-index{}".format(i-no_output_pop-no_input_pop,agent,n_index))
            n_pop_list.append(p.Population(n_number, p.IF_cond_exp(), label=n_pop_labels[i]))
...
def seperate_test(agent, angle, distance, continuous):
    ...
    global big_experimental_record
    #reset and run
    for i in range(parallel_agents):
        current_angle[i] = angle
        current_beam_vlct[i] = 0
        current_beam_acc[i] = 0
        current_position[i] = distance
        current_ball_vlct[i] = 0
        current_ball_acc[i] = 0
        current_agent[i] = agent+i
    if continuous == False:
        p.reset()
    for i in range(parallel_agents):
        for j in range(no_output_pop):
    ...
    for i in range(parallel_agents):
        list_length = len(big_experimental_record[i])
        for j in range(list_length):
            del big_experimental_record[i][0]
        big_experimental_record[i].append([current_position[i], current_ball_vlct[i],current_ball_acc[i],
                                current_angle[i], current_beam_vlct[i], current_beam_acc[i], time.clock()])
    running_status = True
    p.run(duration)
    running_status = False
...
for i in range(parallel_agents):
    big_experimental_record.append([0, 1, 2, 3, 4])
for gen in range(number_of_generations):
    for cycle in range(cycles_per_generation):
        for map_agent in range(map_per_cycle):
            ...
            fitnesses1 = []
            fitnesses2 = []
            for agent in range(0,map_pop_size,parallel_agents):
                print 'starting agent {}-{}/{}/{}/{}'.format(agent, agent+parallel_agents, map_agent, cycle, gen)
